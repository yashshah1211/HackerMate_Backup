...
export default function SIHSpocDashboardModal({
  ...
  }, [isOpen, categoryFilter, statusFilter, searchTerm]);
  ...
  async function handleUpdateSpocStatus(subId: string, status: string, vivaScore?: number | "", notes?: string) {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok && data.success) {
        showToast(`Updated SPOC status for pitch!`, "success");
        setSelectedSub(null);
        fetchSpocData();
      } else {
        showToast(data.error || "Failed to update SPOC status.", "error");
      }
    } catch (err: any) {
      console.error(err);
      showToast(err.message || "Failed to update SPOC status.", "error");
    } finally {
      setUpdating(false);
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-fadeIn">
      <div className="relative w-full max-w-5xl bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl overflow-hidden max-h-[92vh] flex flex-col transition-colors">
        ...
        <div className="p-6 space-y-6 overflow-y-auto flex-1 text-xs">
          ...
          {stats && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="p-3.5 rounded-xl bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800">
                <div className="text-xl font-bold text-zinc-900 dark:text-white">{stats.totalTeams}</div>
                <div className="text-[11px] text-zinc-500 font-mono font-medium">Total Registered Pitches</div>
              </div>
              <div className="p-3.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-500/30">
                <div className="text-xl font-bold text-emerald-700 dark:text-emerald-400">
                  {stats.nominatedSoftware}
                </div>
                <div className="text-[11px] text-emerald-800 dark:text-emerald-300 font-mono font-medium">
                  Software Edition Approved
                </div>
              </div>
              <div className="p-3.5 rounded-xl bg-sky-50 dark:bg-sky-950/20 border border-sky-200 dark:border-sky-500/30">
                <div className="text-xl font-bold text-sky-700 dark:text-sky-400">
                  {stats.nominatedHardware}
                </div>
                <div className="text-[11px] text-sky-800 dark:text-sky-300 font-mono font-medium">
                  Hardware Edition Approved
                </div>
              </div>
              <div className="p-3.5 rounded-xl bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-500/30">
                <div className="text-xl font-bold text-rose-700 dark:text-rose-400">{stats.ruleViolations}</div>
                <div className="text-[11px] text-rose-800 dark:text-rose-300 font-mono font-medium">
                  Rule Violations (Missing Female / Squad)
                </div>
              </div>
            </div>
          )}
          ...
          {/* Filters Bar */}
          <div className="flex flex-wrap items-center justify-between gap-3 p-3 rounded-xl bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800">
            <div className="flex flex-wrap items-center gap-3">
              <input
                type="text"
                placeholder="Search team, PS ID, or Title..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="px-3 py-1.5 bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white placeholder-zinc-400 font-mono text-xs focus:outline-none focus:border-emerald-500"
              />
            ...
            <span className="text-[11px] text-zinc-500 font-mono">
              Showing {submissions.length} Teams
            </span>
          </div>
          ...
          {loading ? (
            <div className="py-12 text-center text-zinc-500 flex flex-col items-center gap-2">
              ...
              <span>Loading SPOC Internal Master Sheet...</span>
            </div>
          ) : submissions.length === 0 ? (
            <div className="py-12 text-center text-zinc-500 bg-zinc-50 dark:bg-zinc-900/40 rounded-xl border border-zinc-200 dark:border-zinc-800">
              No matching team pitches found for the selected filters.
            </div>
          ) : (
            <div className="overflow-x-auto border border-zinc-200 dark:border-zinc-800 rounded-xl">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-zinc-100 dark:bg-zinc-900/80 text-zinc-700 dark:text-zinc-300 font-mono text-[11px] border-b border-zinc-200 dark:border-zinc-800">
                    <th className="p-3">Rank & Team</th>
                    <th className="p-3">PS ID & Title</th>
                    <th className="p-3">Squad Compliance</th>
                    ...
                    <th className="p-3 text-center">SPOC Status</th>
                      ...
                      <tr key={sub.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/40 transition">
                        ...
                        <td className="p-3 max-w-xs">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="px-1.5 py-0.2 rounded text-[9px] uppercase font-mono bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300">
                              {sub.ps_category}
                            </span>
                          </div>
                          <div className="truncate font-medium text-zinc-800 dark:text-zinc-200 mt-0.5" title={sub.ps_title}>
                            {sub.ps_title}
                          </div>
                        </td>
                        ...
                        <td className="p-3 text-center">
                          <span
                            className={`px-2.5 py-1 rounded-full text-[10px] font-mono font-bold border ${getSpocBadgeClass(
                        ...
                        <td className="p-3 text-right">
                          <button
                            ...
                            className="px-3 py-1 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg transition text-[11px] cursor-pointer shadow-sm"
                          >
                            Manage SPOC →
                          </button>
                        </td>
              ...
              <div className="space-y-4 text-xs">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-lg p-2 font-mono text-sm text-zinc-900 dark:text-white placeholder-zinc-500"
                  />
                  <span className="text-[10px] text-zinc-500 font-mono mt-1 block">
                    Composite Score: {Math.round((selectedSub.total_score || 0) * 0.6 + numericViva * 0.4)} pts (60% AI + 40% Viva)
                  </span>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-lg p-2 text-zinc-900 dark:text-white"
                  />
                </div>
                ...
                <div className="space-y-2 pt-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <button
                      ...
                      disabled={updating}
                      className="p-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-bold transition text-center cursor-pointer shadow-md"
                    >
                      ✅ Approve Pitch
                    </button>
                    <button
                      ...
                      disabled={updating}
                      className="p-2.5 rounded-lg bg-amber-600 hover:bg-amber-500 text-white font-bold transition text-center cursor-pointer shadow-md"
                    >
                      ⚠️ Pitch Revision
                    </button>
                    <button
                      ...
                      disabled={updating}
                      className="p-2.5 rounded-lg bg-rose-600 hover:bg-rose-500 text-white font-bold transition text-center cursor-pointer shadow-md"
                    >
                      🚨 Flag / Reject
                    </button>
                  </div>
